...
local function get_module_internals()
  ...
  -- Access private functions by reloading and monkey-patching
  local module_path = "lua/tiny-glimmer/animation/diff_based.lua"
  local content = table.concat(vim.fn.readfile(module_path), "\n")
  
  -- Extract private functions by parsing
  local find_longest_common_substring = function(before, after)
    local max_len = 0
  ...
  return {
    find_longest_common_substring = find_longest_common_substring,
...
-- Test LCS algorithm
T["diff_based"]["LCS finds common substring in middle"] = function()
  local internals = get_module_internals()
  local before = "<h1>hello</h1>"
  local after = "<h1>hello</h1>"
  
  local before_start, after_start, len = internals.find_longest_common_substring(before, after)
  
  -- Should find entire string as common
  MiniTest.expect.equality(len, #before)
end

T["diff_based"]["LCS with surround operation - add quotes"] = function()
  local internals = get_module_internals()
  local before = "hello"
  local after = '"hello"'
  ...
  -- Should find "hello" as the common substring
  MiniTest.expect.equality(len, 5) -- "hello" length
  MiniTest.expect.equality(before_start, 0) -- "hello" starts at 0 in before
  MiniTest.expect.equality(after_start, 1) -- "hello" starts at 1 in after (after opening quote)
end

T["diff_based"]["LCS with HTML tag wrap"] = function()
  local internals = get_module_internals()
  local before = "hello"
  local after = "<h1>hello</h1>"
  ...
  -- Should find "hello" as the common substring
  MiniTest.expect.equality(len, 5)
  MiniTest.expect.equality(before_start, 0)
  MiniTest.expect.equality(after_start, 4) -- "hello" starts after "<h1>"
end

T["diff_based"]["LCS with no common substring"] = function()
  local internals = get_module_internals()
  local before = "abc"
  local after = "xyz"
  ...
  MiniTest.expect.equality(len, 0)
end

T["diff_based"]["LCS with empty before string"] = function()
  local internals = get_module_internals()
  local before = ""
  local after = "hello"
...
T["diff_based"]["find_changed_ranges with character in middle"] = function()
  ...
  -- This should find the longest common substring and mark changes
  -- Depending on LCS, might find "hel" or "lo" as longest
  -- Let's verify behavior is sensible
  MiniTest.expect.equality(#ranges >= 1, true)
end
