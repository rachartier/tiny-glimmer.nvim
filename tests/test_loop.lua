...
describe("Loop functionality", function()
	...
	it("should support finite loop count", function()
	...
	it("should support infinite loops that can be stopped", function()
		local glimmer = require("tiny-glimmer.lib")
		...
		eq(factory.buffers[buffer].named_animations["test_infinite"], nil)
	end)

	it("should loop the specified number of times", function()
		...
		eq(loop_count >= 2, true)
	end)

	it("should not loop when loop is false", function()
		local glimmer = require("tiny-glimmer.lib")
		local completed = false
